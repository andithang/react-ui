...
export type PagingParameters = {
  ...
  onChangePagingParams: (
    eventValue: ChangePagingParamsType,
    key: 'search' | 'pageIndex' | 'pageSize' | 'pageIndexAndSize' | string
  ) => void;
  searchBoxConfig?: {
    placeholder?: string;
    style?: CSSProperties;
...
export const ItemActionEventName = {
  CREATE: 'create',
  IMPORT: 'import',
  EXPORT: 'export',
  DELETE_TEMP_LIST: 'deleteTempList',
  RECOVER_LIST: 'recoverList',
...
export type CustomProTableProps<TData, TStatus, TTabCondition> = {
  ...
  vtsScroll?: { x: string | null; y: string | null };
